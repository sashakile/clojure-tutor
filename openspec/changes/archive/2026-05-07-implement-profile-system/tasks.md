...
- [x] 0.1 Close `CLT-29m` — the Phase 0 spikes achieved 100% action-observation
...
- [x] 1.5 Update `src/editor/cell.js`: replace hardcoded `ACTIVE_PROFILE` and
...
- [x] 2.1 Create `src/profiles/switch.js` exporting `applyProfileSwitch(newId,
...
- [x] 2.2 Update `src/editor/shell.js` to maintain a `cells` array; subscribe to
...
- [x] 3.3 Create `src/nav/nav-layer.js` exporting `installNavLayer(shell, panel,
...
- [x] 3.4 Mount nav-layer, profiles-panel, and which-key overlay in `shell.js`
...
- [x] 4.2 Create `src/lesson/fixture.js` exporting `createFixture(container)`:
...
- [x] 4.3 Mount fixture in `shell.js` by calling `createFixture` before the
...
- [x] 5.3 Test profile switch effects (`src/profiles/switch.js`): after
...
- [x] 5.7 Test nav-layer interception (`src/nav/nav-layer.js`): leader fires when
